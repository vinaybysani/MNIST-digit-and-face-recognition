...
def enhancedFeatureExtractorDigit(datum):
    ...
    1. no. of breaks in horizontal and vertical spaces
    2. no. of black pixels
    3. % of white pixels above the horizontal center line
    4. % of white pixels below the horizontal center line
    5. % of white pixels to the right of the vertical center line
    6. % of white pixels to the left of the vertical center line
    ...
    nonZeroPixels = 0
    firstLeft = None
    pixelsAboveTheCenter = 0
    pixelsBelowtheCenter = 0
        ...
        for j in range(len(row)):
            if row[j] != 0:
                ...
                if not firstLeft or j < firstLeft:
                    firstLeft = j
                if j <= (len(pixels) + 1) / 2:
                    pixelsAboveTheCenter += 1
                else:
                    pixelsBelowtheCenter += 1
            if row[j] != row[j - 1]:
                breaks += 1

    j = 0
    pixelsRightToCenter = 0
    pixelsLeftToCenter = 0
        ...
        while i < len(col):
            if col[j] != 0:
                ...
                if i <= (len(pixels[0]) + 1) / 2:
                    pixelsRightToCenter += 1
                else:
                    pixelsLeftToCenter += 1
            if col[i] != row[i - 1]:
                breaks += 1
            i += 1
        j += 1
    ...
    for n in range(0, 5):
        features[n] = breaks > 150 and 1.0 or 0.0

    for n in range(5):
        features[-n] = nonZeroPixels > 310 and 1.0 or 0.0

    percentAboveCenter = float(pixelsAboveTheCenter) / nonZeroPixels
    for n in range(5):
        features[-(n + 1) * 10] = percentAboveCenter > 0.30 and 1.0 or 0.0

    percentBelowCenter = float(pixelsBelowtheCenter) / nonZeroPixels
    for n in range(5):
        features[(n + 1) * 10] = percentBelowCenter > 0.30 and 1.0 or 0.0

    percentRightToCenter = float(pixelsRightToCenter) / nonZeroPixels
    for n in range(1000, 1005):
        features[n] = percentRightToCenter < 0.27 and 1.0 or 0.0

    percentLeftToCenter = float(pixelsLeftToCenter) / nonZeroPixels
    for n in range(1006, 1010):
        features[n] = percentLeftToCenter < 0.27 and 1.0 or 0.0
